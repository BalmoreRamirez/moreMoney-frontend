<template>
  <section>
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold" style="color:var(--color-text-primary)">Egresos</h1>
        <p class="mt-1 text-sm" style="color:var(--color-text-secondary)">Gastos manuales registrados en tus cuentas.</p>
      </div>
      <button class="btn-brand" @click="abrirCrear">
    ...
    <div v-if="egresosFiltrados.length" class="mt-4 flex items-center justify-between rounded-xl px-5 py-3" style="background:rgba(220,38,38,0.08);border:1px solid rgba(220,38,38,0.15)">
      <span class="text-sm" style="color:var(--color-text-secondary)">Total del período</span>
      <span class="font-mono text-lg font-bold" style="color:var(--color-danger)">{{ formatCurrency(totalPeriodo) }}</span>
    </div>
    ...
    >
      ...
      <Column field="fecha" header="Fecha" sortable style="min-width:110px">
        <template #body="{ data: e }">
          <span class="text-sm" style="color:var(--color-text-secondary)">{{ formatDate(e.fecha?.split('T')[0] ?? '') }}</span>
        </template>
      </Column>
      ...
      <Column field="descripcion" header="Descripción" sortable style="min-width:200px">
        <template #body="{ data: e }">
          <p class="text-sm truncate" style="color:var(--color-text-primary)">{{ e.descripcion }}</p>
          <span class="mt-0.5 inline-block rounded-md px-2 py-0.5 text-xs" style="background:var(--color-surface-mid);color:var(--color-text-muted)">
            {{ e.cuenta?.nombre || '—' }}
          </span>
        </template>
      </Column>
      ...
      <Column field="monto" header="Monto" sortable style="min-width:120px">
        <template #body="{ data: e }">
          <span class="font-mono font-semibold" style="color:var(--color-danger)">{{ formatCurrency(e.monto) }}</span>
        </template>
      </Column>
      ...
      <Column header="" style="min-width:90px;width:90px">
        <template #body="{ data: e }">
          <div class="flex items-center justify-end gap-1">
            <button class="icon-btn" title="Editar" @click="editarEgreso(e)">
              <span class="material-symbols-outlined text-[16px]">edit</span>
            </button>
            <button class="icon-btn-danger" title="Eliminar" @click="confirmarEliminar(e)">
    ...
    <Teleport to="body">
      <Transition name="modal">
        <div v-if="egresoEliminar" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="egresoEliminar = null">
          ...
          <div class="relative w-full max-w-sm rounded-2xl p-6 shadow-card" style="background:var(--color-surface);border:1px solid rgba(220,38,38,0.25)">
            <p class="text-base font-semibold" style="color:var(--color-text-primary)">¿Eliminar egreso?</p>
            <p class="mt-1 text-sm" style="color:var(--color-text-secondary)">{{ egresoEliminar?.descripcion }} — {{ formatCurrency(egresoEliminar?.monto) }}</p>
            <div class="mt-5 flex gap-3">
              <button class="flex-1 rounded-xl py-2.5 text-sm" style="color:var(--color-text-muted);background:var(--color-surface-mid)" @click="egresoEliminar = null">Cancelar</button>
              <button
