<template>
  <section>
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-100">Tarjetas</h1>
    ...
    <div v-else class="mt-10 fintech-card flex flex-col items-center justify-center p-16 text-center">
      ...
      <p class="mt-3 font-semibold text-slate-300">No tienes tarjetas registradas</p>
    ...
    <div v-if="store.totalPages > 1" class="mt-6 flex items-center justify-center gap-2">
      <button
        class="flex h-8 w-8 items-center justify-center rounded-lg text-sm text-slate-400 transition-colors disabled:opacity-30 hover:bg-white/10"
        ...
        <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4" @mousedown.self="deleteTarget = null">
          ...
          <div class="relative w-full max-w-sm rounded-2xl p-6 shadow-card" style="background: #0D2240; border: 1px solid rgba(255,255,255,0.1)">
            <div class="flex items-start gap-4">
              ...
              <div>
                <h3 class="font-semibold text-white">Eliminar tarjeta</h3>
                <p class="mt-1 text-sm text-slate-400">
                  ¿Eliminar <strong class="text-white">{{ deleteTarget?.nombre }}</strong>? Esta acción no se puede deshacer.
                </p>
                <p v-if="deleteError" class="mt-2 text-xs text-danger">{{ deleteError }}</p>
              </div>
            </div>
            <div class="mt-5 flex gap-3">
              <button class="flex-1 rounded-xl py-2.5 text-sm font-medium text-slate-400 hover:bg-white/5" @click="deleteTarget = null">Cancelar</button>
              <button
