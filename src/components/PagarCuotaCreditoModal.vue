<template>
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <h2 class="text-base font-semibold text-white">Pagar cuota</h2>

        <div v-if="cuota" class="mt-3 rounded-xl px-4 py-3 text-xs space-y-1.5" style="background:rgba(255,255,255,0.04)">
          <div class="flex justify-between text-slate-500">
            <span>Cuota</span>
            <span class="text-slate-300">{{ cuota.numero_cuota }} / {{ credito?.num_cuotas }}</span>
          </div>
          <div class="flex justify-between text-slate-500">
            <span>Capital</span>
            <span class="font-mono text-slate-300">{{ formatCurrency(cuota.capital_cuota) }}</span>
          </div>
          <div class="flex justify-between text-slate-500">
            <span>Interés</span>
            <span class="font-mono" style="color:#FBBF24">{{ formatCurrency(cuota.interes_cuota) }}</span>
          </div>
          <div class="flex justify-between text-slate-500 pt-1" style="border-top:1px solid rgba(255,255,255,0.06)">
        ...
        <form class="mt-4 flex flex-col gap-4" @submit.prevent="submit">
          <div>
            <label class="mb-1.5 block text-xs text-slate-400">Cuenta de donde sale el pago</label>
          ...
          <div>
            <label class="mb-1.5 block text-xs text-slate-400">Fecha de pago</label>
