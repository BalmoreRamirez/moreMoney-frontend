<template>
  <Teleport to="body">
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <h2 class="text-base font-semibold text-white">Registrar abono</h2>
        <p v-if="prestamo" class="mt-1 text-xs text-slate-400">
          Deudor: <span class="text-slate-300">{{ prestamo.deudor_nombre }}</span> —
          Saldo pendiente: <span class="font-mono" style="color:#FBBF24">{{ formatCurrency(prestamo.saldo_pendiente) }}</span>
        </p>

        <form class="mt-5 flex flex-col gap-4" @submit.prevent="submit">
          <div>
            <label class="mb-1.5 block text-xs text-slate-400">Monto del abono ($)</label>
          ...
          <div>
            <label class="mb-1.5 block text-xs text-slate-400">Nota (opcional)</label>
